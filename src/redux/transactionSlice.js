...
export const getAllByAdmin = createAsyncThunk(
  'adminlogs/getAll',
  async (_, { rejectWithValue }) => {
    try {
      const response = await transactionService.getAllByAdmin()
      return response
    } catch (error) {
      return rejectWithValue(error?.response?.data)
    }
...
const slice = createSlice({
  ...
  extraReducers: {
    ...
    [getAllByAdmin.pending]: (state) => {
      if (state.data.length <= 0) {
        state.loading = true
      }
    },
    [getAllByAdmin.fulfilled]: (state, action) => {
      state.error = false
      state.data = action.payload
      state.loading = false
    },
    [getAllByAdmin.rejected]: (state, action) => {
      state.error = true
      state.message = action.payload
      state.loading = false
    },
